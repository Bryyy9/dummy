...
export function SearchResults({ results, onResultClick }: SearchResultsProps) {
  ...
  return (
    <div>
      <h4 className="font-semibold text-[#003b5c] mb-3 flex items-center gap-2">
        ...
        {results.map((result, index) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <h5 className="font-medium text-[#003b5c] flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-1">
              {result.culturalElements.slice(0, 2).map((element, idx) => (
                <Badge key={idx} variant="outline" className="text-xs border-[#00a3e0]/30">
                  {element}
                </Badge>
              ))}
