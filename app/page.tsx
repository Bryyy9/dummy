...
function IndonesianGlobe({ onGlobeClick }: { onGlobeClick: () => void }) {
  ...
  return (
    <Float speed={1.2} rotationIntensity={0.4} floatIntensity={0.6}>
      <group scale={1.2}>
        {" "}
        {/* Tambahkan scale pada group utama */}
...
export default function CulturalHeritagePage() {
  ...
  const handleGlobeClick = () => {
    window.location.href = "/peta-budaya"
  }
                ...
                {(searchQuery || selectedCategory !== "semua") && (
                  <div className="text-sm text-muted-foreground">
                    {displayItems.length > 0
                      ? `Ditemukan ${displayItems.length} hasil${searchQuery ? ` untuk "${searchQuery}"` : ""}${
